...
def generate_matrix_inverse(dir, n, data_width, bin_pos):
    ...
    with open(dir + rf"/matinv{n}.v", 'w') as out_file:
        ...
        verilog_write(out_file, r"wire lu_ready;")
        verilog_write(out_file, r"wire lu_complete");

        verilog_write(out_file, r"wire singular;")

        verilog_write(out_file, r"wire [(MATRIX_SIZE * MATRIX_SIZE * DATA_WIDTH) - 1:0] lu;")
        verilog_write(out_file, r"wire [((MATRIX_SIZE + 1) * DATA_WIDTH) - 1:0] p;")

        verilog_write(out_file, rf"matlu{n} #(.DATA_WIDTH(DATA_WIDTH), .BIN_POS(BIN_POS), .MATRIX_SIZE(MATRIX_SIZE)) m1(clk, rst, lu_ready, lu_complete, a, lu, p, singular);")

        verilog_write(out_file, r"always @(posedge clk)")
        verilog_write(out_file, r"begin")
        verilog_write(out_file, r"if (rst)")
        verilog_write(out_file, r"begin")
        verilog_write(out_file, r"endmodule")
        verilog_write(out_file, r"end")
