...
The 1975 MOS Technology 6502 consisted of approximately 3500 transistors. This is a comprehensible size, in that it is entirely possible to imagine it being designed by hand. On the other hand, the first CPU to introduce the x86 instruction set was the Intel 8086 back in 1978, made from some 29,000 transistors. An Intel Nehalem CPU from 12 years ago contains upwards of 750 million transistors. AMD's ``Epyc'' lineup of server CPUs contain up to 50 billion transistors per CPU. These would either be difficult and time consuming to design by hand (in the case of the 8086), or downright impossible to design by hand (the modern Nehalem and Epyc Rome CPUs). Thus the modern approach to chip design \change[is based off using]{requires the use of} Hardware Description Languages (HDLs).
\notedme{While I agree with the specifics, it feels slightly like you're presenting the argument backwards: the only reason that chips have grown to use that number of transistors is because HDLs and other `compilation' tools were developing alongside the release of the chips. In all CS areas there has been development of tools to raise efficiency and abstraction convenience, so it's not likely to be surprising to readers that this happens in chip design too. No change needed, though, just providing a reaction to how the argument seems to be being presented!}
...
\notedme{why is it only fairly well understood?}
...
Such a bad\notedme{Not clear that pejorative wording is required: it's minimalistic rather than `bad' I'd say?} CPU\footnote{I want emphasize here just how bad this CPU is---inefficient, doesn't support a stack, no registers, no signed integers, no floating point numbers etc.\@---it is just a toy to show what can be done with HDLs such as Verilog} might be similar to the one included below:
\notedme{Is this your code? Not clear. If not, it's not entirely clear what you expect readers to take away from giving the listing?}

\notedme{I think you can probably step the font-size down to small, and it will still be readable.}
...
In other words, it is a perfectly functioning CPU with its \notedme{Was incorrect as `it's'---systematic scanning of the whole document for `its' and `it's' is a good polishing step.} own machine language code, which supports port-mapped \change[IO]{I/O} (at a stretch)\notedme{meaning?}, unconditional and conditional branching, and basic unsigned integer arithmetic.
...
\notedme{Minor suggestion, but I'd reword to make the flow simpler, e.g., more like ``To work efficiently with HDLs, you need a software-based simulator to test and debug your code. I use Icarus Verilog.''}

\subsection{Hardware vs.\@ Software Considerations}

Many common operations on large matrices can be defined recursively, such as the determinant of an $n \times n$ matrix being a function of the determinants of its $(n-1) \times (n-1)$ minor matrices. If we consider imperative programming in a language such as C (again assuming that we are targeting a conventional architecture such as ARM or x86), then we try to avoid recursion as much as possible, favouring iterative approaches, since they tend to avoid some of the huge blow outs in runtime that can occur, since many recursive algorithms often calculate the same value multiple times. 
\notedme{Still not entirely sure readers will swallow this. Feels more like an algorithm switch than a problem with recursion \emph{per se}. The reason the iterative approach implemented similarly would have a lower complexity, would probably largely just because developers would tend to end up something akin to memoisation. For example, if avoiding double recursion, it's quite possible to directly turn recursion into iteration with tail-call optimisation / last-call optimisation.}
One example of this is the Fibonacci sequence:
\begin{eqnarray}
	F_0&&= 0 \\
	F_1&&= 1 \\
	F_n&&= F_{n-1} + F_{n-2}
...
\notedme{But you're not saying anything positive about recursion in the software example, so I'm not sure how the topic sentence is intended to preformat readers' minds?}

These stem from the inherently sequential nature of software and the inherently parallel nature of hardware. It would be entirely possible to parallelise the matrix determinant code to use multiple processors / multiple cores, however this would introduce several challenges, thus the simple sequential program is the ``default'' (i.e.\@ most intuitively sensible option). If we now consider the hardware option, it would be possible for it to only have only one arithmetic unit, and rewrite the code such that only one mathematical operation is happening at any given time. However, this would introduce considerable complexity since we would need to store state (how far through the calculation we are), and the unit would need to be clocked---effectively turning it \notedme{What's `it'?} into a very specific CPU. Thus the parallel implementation can be considered the ``default'' for hardware.
...
\begin{figure}[thp]
	...
	\caption{A decent estimate of the orientation given noisy data. The covariance grows slowly.\notedme{Check for consistency regarding style for full-stops on captions: I think you might be putting full-stops for multi-sentence captions (fine if so!), but there's a chance that this inconsistency was accidental.}}
	\label{3d_orient_fig}
\end{figure}
...
As we have seen in \ref{recursion_space}, the amount of Verilog code required to perform a simple operation is quite vast. 
\notedme{`simple operation' here is too underspecified: for some simple operations, the Verilog won't be that bad.}
The $4\times4$ code I show could be written by hand fairly easily, however this would not be possible for the $12 \times 12$ matrix required for three dimensions each of orientation, acceleration, angular velocity, and the magnetic field vector---even a $9 \times 9$ version took up 40 slides in a beamer.
...
\begin{itemize}
	...
	\item Verifying that it \notedme{de-it it} operates correctly, by comparing it to the Python implementation---this would take less than one week, and would mainly involve adding some method to log data from the FPGA to a computer, e.g.\@ via a UART module.
