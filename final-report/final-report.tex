...
As we have seen the naive matrix algorithms are very inefficient, suffering from factorial complexity, typically considered to be one of the worse possible levels of algorithmic complexity. For context, ``bogosort'' (sorting a list by shuffling it until it is sorted by chance) runs in factorial time\footnote{In the average case that is. In the best case (if the list is already sorted), bogosort's performance is $\mathcal{O}(n)$, since checking if it is sorted takes linear time. The worst case performance is unbounded since bogosort is not guaranteed to ever complete.}. Thankfully there are better ways\footnote{This is very common with numerical methods where often the more intuitive methods are mathematically ``cleaner'', however they tend to be slower or more numerically unstable than the less intuitive but more optimised algorithms. For example, the QR factorisation---which is briefly discussed later---decomposes a matrix into an orthogonal matrix $\mathbf{Q}$ and an upper triangular matrix $\mathbf{R}$. The usual process for orthogonalization is the Gram–Schmidt process, which projects vectors onto an orthonormal basis. Unfortunately the Gram–Schmidt process is numerically unstable so Householder reflections are often used instead, which are conceptually less intuitive.}. Imagine we had two special matrices: A lower triangular matrix $\mathbf{L}$, and an upper triangular matrix $\mathbf{U}$, both square matrices with size $n \times n$. Let's also imagine that the values on the diagonal of $\mathbf{L}$ are all 1. We can explore how these matrices have much better properties for finding the determinant and inverse of their product, $\mathbf{LU}$.
...
\begin{figure}[thp]
	...
	\caption{The sort of FPGA I would target for this project. Image courtesy of Tim Molteno.}
	\label{tim_xilinx}
\end{figure}
